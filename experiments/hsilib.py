    ...
    if len(HSI.shape) == 3:
        ...
        for i in range(height):
            for j in range(width):
                IMFs[:, i, j, :] = np.float32(do_EMD(np.float64(HSI[i, j]), number_of_modes + start_mode)[start_mode:])
        ...
        for i in range(size):
            IMFs[:, i, :] = np.float32(do_EMD(np.float64(HSI[i]), number_of_modes + start_mode)[start_mode:])
